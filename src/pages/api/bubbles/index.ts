...
import {DailyRoomPrivacy, DailyRoomSendPermission} from "@/lib/daily/interfaces";
import slugify from "slugify";
import {createBubble, deleteBubble, getBubbleById} from "@/lib/db/services/bubble";
...
class BubblesHandler {
    @Post("/")
    @JwtAuthGuard()
    public async create(@Req() req: NextApiRequest, @Body(ValidationPipe) body: CreateBubbleDTO) {
        const {
            name,
            farcasterCastHash,
        ...
        const newDailyRoom = await dailyHelper.createRoom(bubbleSlug, privacyType === BubblePrivacyType.OPEN ? DailyRoomPrivacy.PUBLIC : DailyRoomPrivacy.PRIVATE, {
